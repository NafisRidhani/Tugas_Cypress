...
describe('From input', () => {
    ...
    it('Fill Inline Forms',() => {
        cy.get('input[placeholder="Jane Doe"]').type('M. Nafis Ridhani')
        cy.get('[placeholder ="Email"][type="text"]').type('nafisridhani@gmail.com')
        cy.get('.custom-checkbox').first().click()
    });

    it('Using Grid',()=> {
        cy.get('#inputEmail1').type('nafisridhani@gmail.com')
    ...
    it('Basic Form',()=> {
        cy.get('#exampleInputEmail1').type('nafisridhani@gmail.com')
        cy.get('#exampleInputPassword1').type('123456789')
        cy.get('.text').contains('Check me out').click()
    })

    it('Form Without Labels',()=>{
        cy.get('[placeholder="Recipients"]').type('M. Nafis Ridhani')
        cy.get('[placeholder="Subject"]').type('Tugas PPL BlackBoxTesting')
        cy.get('[placeholder="Message"').type('Selamat Mengerjakan')
    })

    it('Block Form',()=>{
        cy.get('#inputFirstName').type('Nafis')
        cy.get('#inputLastName').type('Ridhani')
        cy.get('#inputEmail').type('nafisridhani@gmail.com')
        cy.get('#inputWebsite').type('www.wibusubs.com')
    })

    it('Horizontal Form',()=> {
        cy.get('#inputEmail3').type('nafisridhani@gmail.com')
